...
class SiTNet_Unet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=96, kernel_size=3, reduction=4, bias=False, cab=True,
        ...
        super(SiTNet_Unet, self).__init__()
        self.mlp_ratio = mlp_ratio
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=128, embed_dim=128,
        ...
        self.tail = conv(embed_dim, out_c, kernel_size, bias=bias)
    ...
    def forward(self, x3_img):
        ...
        # x3_cat = [2, 128, 256, 256]
        # [2, 16384, 128]
        x3_cat = self.patch_embed(x3_cat)
        x3_cat = self.pos_drop(x3_cat)
        #
        x3_cat = self.rstb(x3_cat, x3_size)
